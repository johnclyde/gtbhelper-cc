...
// Import division manager functions
import { 
  getDivisionCounts, 
  updateDivisionCount, 
  updateSanyakuCount,
...
test('should handle negative division counts', () => {
...
test('should handle extremely large division counts', () => {
...
test('should handle invalid division names', () => {
  // Should not throw
...
test('should handle invalid sanyaku ranks', () => {
  // Should not throw
...
test('should handle adding division that already exists', () => {
  // Add juryo when it already exists
  addDivision('oldBanzuke', 'juryo');
  const counts = getDivisionCounts('oldBanzuke');
  assertEquals(counts.J, 14, 'Should maintain existing count');
});

test('should handle removing division that does not exist', () => {
  // Remove makushita when it's already 0
  removeDivision('oldBanzuke', 'makushita');
  const counts = getDivisionCounts('oldBanzuke');
  assertEquals(counts.Ms, 0, 'Should remain at 0');
});
...
test('should handle switching between banzuke types', () => {
  // Update old banzuke
  updateDivisionCount('oldBanzuke', 'maegashira', 20);
  ...
  assertEquals(oldCounts.M, 20, 'Old banzuke should have its own count');
  assertEquals(newCounts.M, 25, 'New banzuke should have its own count');
});

test('should handle empty string as banzukeType', () => {
...
test('should validate sanyaku hierarchy', () => {
  ...
  assertEquals(counts.Y, 1, 'Should have 1 Yokozuna');
  assertEquals(counts.O, 5, 'Should allow 5 Ozeki even if incorrect');
});